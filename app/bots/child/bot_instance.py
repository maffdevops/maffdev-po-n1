...
async def _handle_signal_flow(
    ...
    """
    Общая логика по кнопке «Получить сигнал» и после успешной подписки.
    """
    tenant = await _get_tenant(tenant_id)
    if tenant is None:
        await message.answer("Configuration error: tenant not found.")
    ...
    # Полный доступ: регистрация + депозит (если включена проверка депозита)
    full_access = True
    if tenant.check_deposit:
        if not ua.is_registered or not ua.has_deposit:
            full_access = False

    # Если депозиты не проверяем — достаточно дойти до подписки
    if not tenant.check_deposit and tenant.check_subscription:
        is_sub = await _check_subscription(bot, tenant, user_id)
        if not is_sub:
            full_access = False
    ...
    # 2) Регистрация, если включена проверка депозита и пользователь ещё не зарегистрирован
    if tenant.check_deposit and not ua.is_registered:
        await _send_register_screen(message, tenant, lang, user_id)
        return

    # 3) Депозит, если включена проверка депозита и ещё нет депозита
    if tenant.check_deposit and not ua.has_deposit:
        await _send_deposit_screen(message, tenant, lang)
        return

    # Фоллбек — если что-то поменялось в процессе
    key = (tenant_id, user_id)
    if key not in access_welcome_shown:
        access_welcome_shown.add(key)
        await _send_access_open_screen(message, tenant, lang)
